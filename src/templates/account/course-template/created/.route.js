...
routeStore.addRule('toolAccountCourseTemplateCreated', {
  url: (params, ctx) => {
    ...
    if (accountSlug) {
      return `/${accountSlug}/course-template/created${queryString}`;
    }
    if(accountId) {
      return `/account/course-templates/created${queryString}`;
    }
    return `/accounts/me/course-template/created${queryString}`;
  },
  parse: (urlObject) => {
    const params = {};
  ...
  match: (urlObject) => {
    return urlObject.pathname === 'account/course-template/created';
  },
});
